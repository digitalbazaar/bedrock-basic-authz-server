...
export function addOAuth2AuthzServer({
  app, getOAuth2Client = getOAuth2ClientFromConfig
}) {
  const cfg = bedrock.config[NAMESPACE];
...
function _importOAuth2Client({client} = {}) {
  ...
  const {id, allowedScopes, requestableScopes} = client;
  ...
  return {id, allowedScopes: allowedScopes ?? requestableScopes, secretHash};
}
