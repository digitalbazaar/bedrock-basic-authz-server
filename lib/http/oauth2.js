...
export function addOAuth2AuthzServer({app, getOAuth2Client} = {}) {
  const cfg = bedrock.config[NAMESPACE];
  ...
  if(!getOAuth2Client) {
    getOAuth2Client = getOAuth2ClientFromConfig;
  } else {
    // ensure client is validated when using a custom client loader
    const fn = getOAuth2Client;
    getOAuth2Client = async ({clientId} = {}) => _importOAuth2Client({
      client: await fn({clientId})
    });
  }
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      let result;
      try {
        result = await _processAccessTokenRequest({req, getOAuth2Client});
      } catch(error) {
...
export function getOAuth2ClientFromConfig({clientId}) {
...
async function _assertOauth2ClientSecret({client, secret}) {
  // client must have `secretHash`, a base64url-encoded SHA-256 hash of secret
  assert.string(client.secretHash, 'client.secretHash');
...
async function _checkBasicAuthorization({req, getOAuth2Client}) {
  try {
    // parse credentials
...
function _getRequestedScopes({client, request}) {
  ...
  const allowedScopes = client.allowedScopes ?? client.requestableScopes;
...
function _importOAuth2Client({client} = {}) {
  ...
  if(!(client && typeof client === 'object')) {
    throw new TypeError('Invalid oauth2 client; client is not an object.');
  }
  const {baseUri} = bedrock.config.server;
  const {
    id,
  ...
  // handle deprecated `requestableScopes`
  if(requestableScopes) {
    if(allowedScopes) {
      throw new TypeError(
        'Only one of "allowedScopes" or "requestableScopes" is permitted.');
    }
    assert.arrayOfString(requestableScopes, 'client.requestableScopes');
  } else {
    assert.arrayOfString(allowedScopes, 'client.allowedScopes');
  }
  ...
  if(!(audience === baseUri || audience.startsWith(baseUri + '/'))) {
    throw new TypeError(
      'Invalid oauth2 client; ' +
      `"audience" must be or start with "${baseUri}".`);
  }
  return {
    id,
    allowedScopes: allowedScopes ?? requestableScopes,
    audience,
    secretHash
  };
}

async function _processAccessTokenRequest({req, getOAuth2Client}) {
  ...
  // create access token
  const {client} = await _checkBasicAuthorization({req, getOAuth2Client});
  const request = {scope};
